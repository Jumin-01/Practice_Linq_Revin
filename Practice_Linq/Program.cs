...
namespace Practice_Linq
{
    public class Program
    {
        ...
        static void Query1(List<FootballGame> games)
        {
            ...
            {
                Console.WriteLine($"{item.Date.ToShortDateString()} {item.Home_team} - {item.Away_team}, Scope: {item.Home_score} - {item.Away_score}, Country: {item.Country}");
            }
        ...
        static void Query2(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Date.Year >= 2020 && game.Tournament == "Friendly" && game.Home_team == "Italy" | game.Away_team == "Italy"); // Корегуємо запит !!!
        ...
        static void Query3(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Date.Year == 2021 && game.Home_score == game.Away_score && game.Country == "France");  // Корегуємо запит !!!
        ...
        static void Query4(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Date.Year >= 2018 && game.Date.Year <= 2020 && game.Home_score > game.Away_score && game.Away_team == "Germany");   // Корегуємо запит !!!
        ...
        static void Query5(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Tournament == "UEFA Euro qualification" && game.Home_score> game.Away_score && game.City == "Kharkiv"| game.City == "Kyiv");  // Корегуємо запит !!!
        ...
        static void Query7(List<FootballGame> games)
        {
            ...
            FootballGame g = games.Where(game => game.Date.Year == 2023 && (game.Home_team == "Ukraine" || game.Away_team == "Ukraine")).FirstOrDefault(game => (game.Home_team == "Ukraine" && game.Home_score > game.Away_score) || (game.Away_team == "Ukraine" && game.Away_score > game.Home_score));   // Корегуємо запит !!!

            
            // Перевірка
            Console.WriteLine("\n======================== QUERY 7 ========================");          
            Console.WriteLine($"{g.Date.ToShortDateString()} {g.Home_team} - {g.Away_team}, Scope: {g.Home_score} - {g.Away_score}, Country: {g.Country}");
        ...
        static void Query8(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Tournament == "UEFA Euro" && game.Country == "Ukraine");   // Корегуємо запит !!!
            var transformedMatches = selectedGames.Select(match => new
                {
            ...
            // Перевірка
            Console.WriteLine("\n======================== QUERY 8 ========================");
            foreach (var item in transformedMatches)
            {
        ...
        static void Query9(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Tournament == "UEFA Nations League" && game.Date.Year == 2023);// Корегуємо запит !!!
            var transformedMatches = selectedGames.Select(match => new
            {
                MatchYear = match.Date.Year,
                Game = $"{match.Home_team}-{match.Away_team}",
                Result = GetResultForFirstTeam(match.Home_score, match.Away_score)

            }) ;
            string GetResultForFirstTeam(int homeScore, int awayScore)
            {
        ...
        static void Query10(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(game => game.Date.Year==2023 && game.Date.Month == 7 && game.Tournament== "Gold Cup").OrderBy(game => game.Date).Skip(4).Take(6);    // Корегуємо запит !!!
